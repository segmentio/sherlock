...
Detective.prototype.analyze = function(url, fn){
  ...
  for (var integration in integrations) {
    ...
    nightmare.evaluate(settings, function(obj){
      debug('Found key... ' + inspect(obj));
      if (obj) var key = Object.keys(obj)[0];
      if (obj && key) {
        for (var i = 0; i < self.results['integrations'].length; i++){
          if (self.results['integrations'][i].name === key) self.results['integrations'][i].key = obj[key];
        }
      }
    });
  }
