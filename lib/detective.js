      ...
      each(matched, function (integration) {
        ...
        function updateSettings(settings) {
          ...
          if (settings) {
            ...
            nightmare.wait(500).evaluate(integration.settings, updateSettings);
          } else {
...
function matches(sources) {
  ...
  each(sources, function (src) {
    each(integrations, function (integration) {
      if (src.match(integration.pattern)) payload[integration.name] = integration;
    });
  });
  return payload;
}
